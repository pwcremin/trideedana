...
var Lowes = function ()
{
    ...
    this.getListItems = function ( folderId, callback )
    {
        ...
        get( url, function(result, data)
        {
            ...
            {
                var msg = "ERROR: tried to get the list items but fell down instead"
                console.log(msg);
                throw(msg);
            }

            callback(data.list, status);
        } );
    };
    ...
    this.getProductID = function ( keyword, callback )
    {
        ...
        // priceflag=status?
        // storeNumber = 0595
        var url = _host + '/product/keyword?keyword=' + keyword + '&storeNumber=0595&priceFlag=status&maxResults=5&sortMethod=p_product_price&api_key=' + api_key;
        ...
        return fetch( requestUrl, {
            ...
            .then( ( response ) =>
            {
                if(response.status === 403)
                {
                    ...
                    setTimeout(get.bind(this, requestUrl, callback), 1000);
                    return;
                }
    ...
    function post( requestUrl, body, callback )
    {
        ...
        return fetch( requestUrl, {
            ...
            .then( ( response ) =>
            {
                ...
                {
                    console.log("Error: (rate limit?) status is " + response.status);
                    setTimeout(post.bind(this, requestUrl, body, callback), 1000);
                    return;
                }
                response.json().then( function ( json )
                    {
